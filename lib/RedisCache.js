...
module.exports.RedisCache = class RedisCache {
    ...
    constructor(name, options) {
        ...
        this._options.preDestroy ??= async () => {
        };
    ...
    async set(key, value, ttl = this._options.ttl) {
        let entry = await this._connection.get(`cache:${key}`);
        if (entry) {
            await this._options.preDestroy(key, JSON.parse(entry).value, false);
    ...
    async delete(key) {
        let entry = await this._connection.get(`cache:${key}`);
        if (entry) {
            await this._options.preDestroy(key, JSON.parse(entry).value, true);
            await this._connection.del(`cache:${key}`);
        }
    }

    async has(key) {
        return await this._connection.exists(`cache:${key}`) === 1;
    }
    ...
    async clear() {
        ...
        for (let key of keys) {
            ...
            await this._options.preDestroy(key.substring(this._name.length + 1), entry.value, true);
            await this._connection.del(key.substring(this._name.length + 1));
        }
    }
    ...
    async stats() {
        return {
            ...
            hits: Number.parseInt(await this._connection.get(`stats:hits`)),
            miss: Number.parseInt(await this._connection.get(`stats:miss`))
        }
    }
